...
void Weapon_TBolt_Fire(edict_t* ent)
{
	...
	fire_tblaster(ent, start, dir, damage, speed);
...
void weapon_railgrenadelauncher_fire(edict_t* ent)
{
	...
	fire_railgrenade(ent, start, dir, damage, 600, 2.5_sec, radius, (crandom_open() * 10.0f), (200 + crandom_open() * 10.0f), false);
...
void weapon_bfgrenadelauncher_fire(edict_t* ent)
{
	...
	fire_bfgrenade(ent, start, dir, damage, 600, 2.5_sec, radius, (crandom_open() * 10.0f), (200 + crandom_open() * 10.0f), false);
...
void Weapon_GreenFlare_Fire(edict_t* ent)
{
	int damage = 15;
	int speed = 1500;
	...
	fire_greenflare(ent, start, dir, damage, speed);
...
void Weapon_GFlareLauncher(edict_t* ent)
{
	...
	Weapon_Generic(ent, 8, 13, 49, 53, pause_frames, fire_frames, Weapon_GreenFlare_Fire);
}

void Weapon_YellowFlare_Fire(edict_t* ent)
{
	int	  damage;
	float damage_radius;
	int	  radius_damage;
...
void Weapon_YFlareLauncher(edict_t* ent)
{
	...
	Weapon_Generic(ent, 8, 13, 49, 53, pause_frames, fire_frames, Weapon_YellowFlare_Fire);
}

void Weapon_RedFlare_Fire(edict_t* ent)
{
	int	  damage;
	float damage_radius;
	int	  radius_damage;
	...
	vec3_t start, dir;
	P_ProjectSource(ent, ent->client->v_angle, { 8, 8, -8 }, start, dir);
	fire_redflare(ent, start, dir, damage, 650, damage_radius, radius_damage);
...
void Weapon_RFlareLauncher(edict_t* ent)
{
	...
	Weapon_Generic(ent, 8, 13, 49, 53, pause_frames, fire_frames, Weapon_RedFlare_Fire);
}
...
void weapon_homing_bfg_fire(edict_t* ent) //TODO: not homing yet, instead double damage/double cost
{
	...
	// cells can go down during windup (from power armor hits), so
	// check again and abort firing if we don't have enough now
	if (ent->client->pers.inventory[ent->client->pers.weapon->ammo] < 100)
		return;
