...
=================
*/
TOUCH(tblaster_touch) (edict_t* self, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	if (other == self->owner)
		return;
	...
	if (other->takedamage)
	{
		T_Damage(other, self, self->owner, self->velocity, self->s.origin, tr.plane.normal, self->dmg, 1, DAMAGE_ENERGY, MOD_BLASTER2);
	}
	else
	{
		gi.WriteByte(svc_temp_entity);
...
void fire_tblaster(edict_t* self, const vec3_t& start, const vec3_t& dir, int damage, int speed)
{
	edict_t* bolt;
	trace_t	 tr;
...
static void RailGrenade_ExplodeReal(edict_t* ent, edict_t* other, vec3_t normal)
{
	vec3_t origin, dir;
	mod_t  mod;
	int damage, kick;
	...
	if (other)
	{
		vec3_t up = other->s.origin - ent->s.origin;
		if (ent->spawnflags.has(SPAWNFLAG_GRENADE_HAND))
			mod = MOD_HANDGRENADE;
		else
			mod = MOD_GRENADE;
		T_Damage(other, ent, ent->owner, up, ent->s.origin, normal, ent->dmg, ent->dmg, mod.id == MOD_HANDGRENADE ? DAMAGE_RADIUS : DAMAGE_NONE, mod);

		dir = up * frandom(0.25f);
		fire_rail(ent, ent->s.origin, dir, damage, kick);
		dir = up * frandom(0.50f);
		fire_rail(ent, ent->s.origin, dir, damage, kick);
		dir = up * frandom(0.75f);
		fire_rail(ent, ent->s.origin, dir, damage, kick);
		dir = up * frandom(0.25f);
		fire_rail(ent, ent->s.origin, dir, damage, kick);
	}
...
{
	RailGrenade_ExplodeReal(ent, nullptr, ent->velocity * -0.02f);
}
...
TOUCH(RailGrenade_Touch) (edict_t* ent, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	...
	RailGrenade_ExplodeReal(ent, other, tr.plane.normal);
}

THINK(RailGrenade4_Think) (edict_t* self) -> void
{
	if (level.time >= self->timestamp)
	{
...
void fire_railgrenade(edict_t* self, const vec3_t& start, const vec3_t& aimdir, int damage, int speed, gtime_t timer, float damage_radius, float right_adjust, float up_adjust, bool monster)
{
	edict_t* grenade;
	vec3_t	 dir;
	vec3_t	 forward, right, up;
	...
	{
		grenade->avelocity = { crandom() * 360, crandom() * 360, crandom() * 360 };
		grenade->s.modelindex = gi.modelindex("models/objects/railgrenade/tris.md2");
		grenade->nextthink = level.time + timer;
	...
	{
		grenade->s.modelindex = gi.modelindex("models/objects/railgrenade4/tris.md2");
		grenade->s.angles = vectoangles(grenade->velocity);
		grenade->nextthink = level.time + FRAME_TIME_S;
		grenade->timestamp = level.time + timer;
		grenade->think = RailGrenade4_Think;
		grenade->s.renderfx |= RF_MINLIGHT;
	}
	grenade->owner = self;
...
void fire_railgrenade2(edict_t* self, const vec3_t& start, const vec3_t& aimdir, int damage, int speed, gtime_t timer, float damage_radius, bool held)
{
	...
	grenade->s.modelindex = gi.modelindex("models/objects/railgrenade3/tris.md2");
	grenade->owner = self;
	grenade->touch = RailGrenade_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = RailGrenade_Explode;
...
static void BFGrenade_ExplodeReal(edict_t* ent, edict_t* other, vec3_t normal)
{
	vec3_t origin;
	mod_t  mod;
	...
	if (other)
	{
		...
		if (ent->spawnflags.has(SPAWNFLAG_GRENADE_HAND))
			...
			mod = MOD_GRENADE;
		T_Damage(other, ent, ent->owner, dir, ent->s.origin, normal, ent->dmg, ent->dmg, mod.id == MOD_HANDGRENADE ? DAMAGE_RADIUS : DAMAGE_NONE, mod);
		fire_bfg(ent, ent->s.origin, dir, damage, 400, damage_radius);
	}
...
{
	BFGrenade_ExplodeReal(ent, nullptr, ent->velocity * -0.02f);
}
...
TOUCH(BFGrenade_Touch) (edict_t* ent, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	...
	BFGrenade_ExplodeReal(ent, other, tr.plane.normal);
}

THINK(BFGrenade4_Think) (edict_t* self) -> void
{
	if (level.time >= self->timestamp)
	{
...
void fire_bfgrenade(edict_t* self, const vec3_t& start, const vec3_t& aimdir, int damage, int speed, gtime_t timer, float damage_radius, float right_adjust, float up_adjust, bool monster)
{
	edict_t* grenade;
	vec3_t	 dir;
	vec3_t	 forward, right, up;
	...
	{
		grenade->avelocity = { crandom() * 360, crandom() * 360, crandom() * 360 };
		grenade->s.modelindex = gi.modelindex("models/objects/bfgrenade/tris.md2");
		grenade->nextthink = level.time + timer;
	...
	{
		grenade->s.modelindex = gi.modelindex("models/objects/bfgrenade4/tris.md2");
		grenade->s.angles = vectoangles(grenade->velocity);
		grenade->nextthink = level.time + FRAME_TIME_S;
		grenade->timestamp = level.time + timer;
		grenade->think = BFGrenade4_Think;
		grenade->s.renderfx |= RF_MINLIGHT;
	}
	grenade->owner = self;
...
void fire_bfgrenade2(edict_t* self, const vec3_t& start, const vec3_t& aimdir, int damage, int speed, gtime_t timer, float damage_radius, bool held)
{
	...
	grenade->s.modelindex = gi.modelindex("models/objects/bfgrenade3/tris.md2");
	grenade->owner = self;
	grenade->touch = BFGrenade_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = BFGrenade_Explode;
...
TOUCH(yellowflare_touch) (edict_t* ent, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	vec3_t origin;
...
edict_t* fire_yellowflare(edict_t* self, const vec3_t& start, const vec3_t& dir, int damage, int speed, float damage_radius, int radius_damage)
{
	edict_t* yellowflare;

	yellowflare = G_Spawn();
	yellowflare->s.origin = start;
	yellowflare->s.angles = vectoangles(dir);
	yellowflare->velocity = dir * speed;
	yellowflare->movetype = MOVETYPE_FLYMISSILE;
	yellowflare->svflags |= SVF_PROJECTILE;
	yellowflare->flags |= FL_DODGE;
	yellowflare->clipmask = MASK_PROJECTILE;
	// [Paril-KEX]
	if (self->client && !G_ShouldPlayersCollide(true))
		yellowflare->clipmask &= ~CONTENTS_PLAYER;
	yellowflare->solid = SOLID_BBOX;
	yellowflare->s.effects |= EF_ROCKET;
	yellowflare->s.modelindex = gi.modelindex("models/objects/laser/tris.md2");
	yellowflare->owner = self;
	yellowflare->touch = yellowflare_touch;
	yellowflare->nextthink = level.time + gtime_t::from_sec(8000.f / speed);
	yellowflare->think = G_FreeEdict;
	yellowflare->dmg = damage;
	yellowflare->radius_dmg = radius_damage;
	yellowflare->dmg_radius = damage_radius;
	yellowflare->s.sound = gi.soundindex("misc/lasfly.wav");
	yellowflare->classname = "flare";

	gi.linkentity(yellowflare);
...
TOUCH(redflare_touch) (edict_t* ent, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	vec3_t origin;
...
edict_t* fire_redflare(edict_t* self, const vec3_t& start, const vec3_t& dir, int damage, int speed, float damage_radius, int radius_damage)
{
	edict_t* redflare;

	redflare = G_Spawn();
	redflare->s.origin = start;
	redflare->s.angles = vectoangles(dir);
	redflare->velocity = dir * speed;
	redflare->movetype = MOVETYPE_FLYMISSILE;
	redflare->svflags |= SVF_PROJECTILE;
	redflare->flags |= FL_DODGE;
	redflare->clipmask = MASK_PROJECTILE;
	// [Paril-KEX]
	if (self->client && !G_ShouldPlayersCollide(true))
		redflare->clipmask &= ~CONTENTS_PLAYER;
	redflare->solid = SOLID_BBOX;
	redflare->s.effects |= EF_ROCKET;
	redflare->s.modelindex = gi.modelindex("models/objects/laser/tris.md2");
	redflare->owner = self;
	redflare->touch = redflare_touch;
	redflare->nextthink = level.time + gtime_t::from_sec(8000.f / speed);
	redflare->think = G_FreeEdict;
	redflare->dmg = damage;
	redflare->radius_dmg = radius_damage;
	redflare->dmg_radius = damage_radius;
	redflare->s.sound = gi.soundindex("misc/lasfly.wav");
	redflare->classname = "flare";

	gi.linkentity(redflare);
