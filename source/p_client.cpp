...
void InitClientPersistant(edict_t *ent, gclient_t *client)
{
		...
		(!G_TeamplayEnabled() && !client->resp.spectator))
	{
		...
		if (!taken_loadout)
		{
			...
			// ROGUE
			//UNSEEN
			client->pers.max_ammo[AMMO_RAILGRENADES] = 10;
			client->pers.max_ammo[AMMO_BFGRENADES] = 4;
			client->pers.max_ammo[AMMO_GREENFLARES] = 32;
			client->pers.max_ammo[AMMO_YELLOWFLARES] = 16;
			client->pers.max_ammo[AMMO_REDFLARES] = 2;
