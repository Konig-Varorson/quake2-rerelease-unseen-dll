...
void GunnerFire(edict_t *self)
{
	...
	if (strcmp(self->classname, "monster_gunner2") == 0)
		monster_fire_blueblaster(self, start, aim, 3, 800, flash_number, EF_BLUEHYPERBLASTER);
	else
...
void GunnerGrenade(edict_t *self)
{
	...
	if (strcmp(self->classname, "monster_gunner2") == 0)
	{
		if (M_CalculatePitchToFire(self, target, start, aim, 600, 2.5f, false))
			monster_fire_railgrenade(self, start, aim, 50, 600, flash_number, (crandom_open() * 10.0f), frandom() * 10.f);
		else
			// normal shot
			monster_fire_railgrenade(self, start, aim, 50, 600, flash_number, (crandom_open() * 10.0f), 200.f + (crandom_open() * 10.0f));
	}
	else
	{
		if (M_CalculatePitchToFire(self, target, start, aim, 600, 2.5f, false))
